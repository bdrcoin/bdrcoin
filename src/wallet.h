...
enum AvailableCoinsType {
    ALL_COINS = 1,
    ONLY_DENOMINATED = 2,
    ONLY_NOT600000IFMN = 3,
    ONLY_NONDENOMINATED_NOT600000IFMN = 4, // ONLY_NONDENOMINATED and not 600000 BIDR at the same time
    ONLY_600000 = 5                        // find masternode outputs including locked ones (use with caution)
};
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 600000 * COIN) continue; // do not count MN-like outputs
