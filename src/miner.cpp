...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BDRcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BDRcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BDRcoinMiner started\n");
   ...
   // Each thread has its own key and counter
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BDRcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
